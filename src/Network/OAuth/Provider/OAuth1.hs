...
twoLegged tokenLookup secretCreation = do
  ...
  where
    noProcessing = const (return Nothing)
...
threeLeggedRequestTokenRequest = do
  ...
  where
    storeCallback cbStore OAuthParams {..} = do
      _ <- maybe missingCallback (lift . cbStore (opConsumerKey, RequestTokenKey . unToken $ opToken)) opCallback
      return Nothing
...
threeLeggedAccessTokenRequest = do
  ...
  where
    verifierCheck verifierLookup params = do
        (storedVerifier, userId) <- lift $ verifierLookup (opConsumerKey params, RequestTokenKey . unToken . opToken $ params)
        case opVerifier params of
            Just ((==) storedVerifier -> True) -> oauthEither $ Right $ Just userId
...
processTokenCreationRequest :: Monad m =>
    ...
    -> (ConsumerKey -> m (Token, Secret)) -> (OAuthParams -> OAuthM m (Maybe UserId))
...
processTokenCreationRequest tokenLookup secretCreation customProcessing = do
    ...
    case opToken params of
      Token "" -> do
        (Token token, Secret secret) <- lift $ secretCreation $ opConsumerKey params
        _ <- customProcessing params {opToken = Token token}
        return [("oauth_token", token), ("oauth_token_secret", secret)]
      Token _ -> do
        mUserId <- customProcessing params
        (Token token, Secret secret) <- lift $ secretCreation $ opConsumerKey params
        return $ case mUserId of
          Just userId -> [("oauth_token", token), ("oauth_token_secret", secret), ("user_id", userId)]
          Nothing    -> [("oauth_token", token), ("oauth_token_secret", secret)]
